...
3. `references/coding-and-verification-rules.md` — `CLAUDE.md`에서 확정된 코딩 표준(Python 3.14, 품질 지표, `tdd` 스킬 연계)을 11장에 기록하는 방법.
...
- `TPL-SWE3-001` 자체는 코딩 표준을 지정하지 않지만, 이 프로젝트는 `CLAUDE.md`에서 코딩 표준(Python 3.14, 품질 지표, camelCase 네이밍)을 이미 확정했습니다 — `coding-and-verification-rules.md`/`tdd` 스킬을 그대로 따르고 다시 제안하지 않습니다.
