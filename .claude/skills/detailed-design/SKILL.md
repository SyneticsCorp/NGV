...
3. `references/coding-and-verification-rules.md` — 코딩 표준·정적분석·단위검증 기준을 실행 가능하게 정의하는 방법(11장 대응).
...
3. `coding-and-verification-rules.md`로 코딩/검증 규칙을 실행 가능하게 정의합니다.
...
- `TPL-SWE3-001`은 코딩 표준(예: MISRA 등)을 구체적으로 지정하지 않습니다 — 프로젝트가 실제로 채택한 표준이 있는지 먼저 확인하고, 없다면 `coding-and-verification-rules.md`의 절차대로 제안하세요. 지어내지 마세요.
